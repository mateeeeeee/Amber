...
namespace amber
{
	void MedianSplitBuilder::Build(BVHBuildData& data, std::vector<Triangle> const& triangles)
	{
		Uint32 n = static_cast<Uint32>(triangles.size());
		if (n == 0)
		{
			return;
		}

		data.tri_indices.resize(n);
		for (Uint32 i = 0; i < n; i++)
		{
			data.tri_indices[i] = i;
		}

		data.nodes.resize(n * 2 - 1);
		data.nodes_used = 1;
	...
	void MedianSplitBuilder::UpdateNodeBounds(BVHBuildData& data, std::vector<Triangle> const& triangles, Uint32 node_idx)
	{
	...
	void MedianSplitBuilder::Subdivide(BVHBuildData& data, std::vector<Triangle> const& triangles, Uint32 node_idx)
	{
		BVHNode& node = data.nodes[node_idx];
		if (node.tri_count <= 2)
		{
