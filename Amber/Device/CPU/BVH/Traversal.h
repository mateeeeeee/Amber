...
namespace amber
{
	...
	template<Uint32 N>
	Uint32 CountTraversalSteps(BVH<N> const& bvh, Ray const& ray)
	{
		if (bvh.nodes.empty()) 
		{
			return 0;
		...
		while (true)
		{
			...
			if constexpr (N == 2)
			{
				...
				Float dist1 = IntersectAABB(r, child1->aabb_min, child1->aabb_max);
				Float dist2 = IntersectAABB(r, child2->aabb_min, child2->aabb_max);
				if (dist1 > dist2) { std::swap(dist1, dist2); std::swap(child1, child2); }
				if (dist1 == BVH_INFINITY) { if (stack.IsEmpty()) break; node = stack.Pop(); }
				else { node = child1; if (dist2 != BVH_INFINITY) stack.Push(child2); }
			}
			else
			{
				struct ChildHit { BVHNode<N> const* node; Float dist; };
				...
				{
					BVHNode<N> const* child = &bvh.nodes[node->children[i]];
					Float dist = IntersectAABB(r, child->aabb_min, child->aabb_max);
					if (dist != BVH_INFINITY) hits[hit_count++] = { child, dist };
				}
				if (hit_count == 0) { if (stack.IsEmpty()) break; node = stack.Pop(); continue; }
				std::sort(hits, hits + hit_count, [](ChildHit const& a, ChildHit const& b) { return a.dist > b.dist; });
				node = hits[hit_count - 1].node;
				for (Uint32 i = 0; i < hit_count - 1; i++) stack.Push(hits[i].node);
			}
	...
	template<Uint32 N>
	Uint32 CountPrimTests(BVH<N> const& bvh, Ray const& ray)
	{
		if (bvh.nodes.empty()) 
		{
			return 0;
