...
namespace amber
{
	...
	void TopDownBuilder<NodeT, SplitPolicyT>::Build(BVH& bvh, std::span<NodeT const> nodes)
	{
	...
	template<typename NodeT, typename SplitPolicyT> requires TopDownSplitPolicy<NodeT, SplitPolicyT>
	void TopDownBuilder<NodeT, SplitPolicyT>::UpdateNodeBounds(BVH& bvh, NodeT const* nodes, Uint32 node_idx)
	{
	...
	template<typename NodeT, typename SplitPolicyT> requires TopDownSplitPolicy<NodeT, SplitPolicyT>
	void TopDownBuilder<NodeT, SplitPolicyT>::Subdivide(BVH& bvh, NodeT const* nodes, Uint32 node_idx)
	{
		BVHNode& node = bvh.nodes[node_idx];
	...
	template class TopDownBuilder<Triangle,     MedianSplitPolicy>;
	template class TopDownBuilder<Triangle,     BinnedSAHPolicy>;
	template class TopDownBuilder<Triangle,     SweepSAHPolicy>;
	template class TopDownBuilder<BLASInstance, MedianSplitPolicy>;
	template class TopDownBuilder<BLASInstance, BinnedSAHPolicy>;
	template class TopDownBuilder<BLASInstance, SweepSAHPolicy>;
}
